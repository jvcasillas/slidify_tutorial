...
phon  <- c(rep("i", 1), rep("I", 1), rep("E", 1))
freq1  <- floor(runif(10, 0, 20))
freq2  <- floor(runif(10, 15, 30))
freq3  <- floor(runif(10, 25, 40))
freq <- c(freq1, freq2, freq3)

df <- data.frame(Group = group, 
                 Phoneme = phon, 
                 Frequency = freq)
n1 <- nPlot(Frequency ~ Phoneme, group = 'Group', type = 'multiBarChart',
  data = df)
n1$print('chart1')
```
...
```{r opts.label = 'interactive', results = 'asis', echo=FALSE, warning=FALSE, message=FALSE, fig.align='center'}
require(googleVis)
